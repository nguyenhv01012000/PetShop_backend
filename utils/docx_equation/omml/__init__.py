...
_xmlns_str = ' '.join(
    'xmlns:{}="{}"'.format(key, value)
    for key, value in _namespaces.items()
)

_template = string.Template('''<?xml version="1.0" standalone="yes"?>
<w:document mc:Ignorable="w14 w15 w16se wp14" {}>
    $omml_xml
</w:document>'''.format(_xmlns_str))
...
def omml2mml(omml_xml):
    ...
    result = ""
    transform = ET.XSLT(ET.parse(_xslt_filename))
    result = str(transform(tree))
    formated_result = (
        result.replace("mml:", "").replace('<?xml version="1.0"?>', "").strip()
    )
